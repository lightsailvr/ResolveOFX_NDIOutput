...
Its Timeline (Auto) camera-clip source (v1.11.0) bundles two helper scripts into `Contents/Resources/` (the Makefile copies them): `src/ndi_timeline_watch.lua`, which the plugin runs under Resolve's own bundled script interpreter (`DaVinci Resolve.app/Contents/Libraries/Fusion/fuscript -q -l lua`, located from the host executable at runtime — nothing to install, since v1.14.1 / issue #34), and `src/ndi_timeline_watch.py` as the fallback when fuscript is missing, run with the Xcode developer-dir or Homebrew python3 (never `/usr/bin/python3`, which is Apple's Command Line Tools shim and dies silently without the CLT). Both poll the Resolve scripting API for the clip under the playhead (`src/TimelineClipWatcher.cpp`; discovery/spawn seam in `src/MacTimelineWatch.h`, unit-tested by `make test`). Auto mode needs Resolve's external-scripting preference (Preferences → System → General → External scripting using: Local); without it the plugin logs why (including a dead helper's exit status) and Manual Path mode still works. To verify the helper standalone before a plugin install: `"/Applications/DaVinci Resolve/DaVinci Resolve.app/Contents/Libraries/Fusion/fuscript" -q -l lua src/ndi_timeline_watch.lua` prints the playhead clip path twice a second (or a `!` status line explaining why not).
