...
12. **Delay-load the NDI import and resolve it module-relatively at startup** (from the plugin's own module handle). Windows' default loader never searches a DLL's own folder for load-time imports of another DLL, so without this the plugin silently fails to load on any machine without an NDI runtime — the exact machine the bundled DLL exists for. Verified on a machine with no NDI software installed. *(Amended 2026-08-30, ticket #21: the original "add the bundle's directory to the DLL search path" sketch doesn't work — `AddDllDirectory` never reaches the delay-load helper's plain `LoadLibrary`, and `SetDefaultDllDirectories` would mutate Resolve's process-global loader state. The implementation preloads the DLL by full module-relative path instead; see `src/NDIRuntimeLoader.h` and LEARNINGS.md.)*
