...
**Validated by:** Tier 0 — `test_ndi_loader` (path derivation + fallback plumbing) and `test_plugin_delayload` (import stays delay-loaded and lazy) green. Tier 1–2 pass (Matt, 2026-08-31, PR #28): first Windows stream confirmed in Studio Monitor — stereo pairing and Manual-Path STMap working on the CPU-buffer path (performance poor as expected; GPU-native is ticket #22). The no-NDI-machine check is deferred to the release bar (issue #29).
