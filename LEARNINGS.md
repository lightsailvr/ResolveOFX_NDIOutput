...
With the Stereo 3D palette at Vision: Stereo, Resolve creates a **second plugin instance for the right eye**. Both instances create NDI senders with the same default source name; the unified log shows `NDIlib_send_create` failing every render in R/L pair cadence, and `initializeNDI`'s failure path calls **`NDIlib_destroy()`** — killing the process-wide NDI library under the other instance's healthy sender, so the feed goes black and never recovers (L). Duplicate-name registration as the trigger is **confirmed (L, 2026-08-28 v1.4.0 session)**: NDI 6.2 `send_create` fails outright on a name already advertised from the same machine — no auto-suffixing (see the leaked-advertisement entry below). Fix belongs to issue #6: process-shared NDI lifecycle, single sender ownership or eye-suffixed names, and never `NDIlib_destroy()` while another instance is live.
...
**Fix:** none in plugin code yet — this is issue #6's lifecycle work. Session remedies: change the NDI Source Name in the plugin UI (a fresh name registers immediately, no restart), or fully quit Resolve to drop the leaked advertisements.
