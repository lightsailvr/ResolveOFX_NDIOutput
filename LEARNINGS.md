...
**Validated by:** `make test` (117 stereo-pair asserts: hysteresis transitions both directions, parked no-flap, mutual-stall resume, keepalive gating, and an acceptance sweep asserting no single-eye send ever occurs after latch), `make test-metal`, Tier 0. Tier 1–2 pending (needs Resolve restart + human playback): verify Stream Status holds steady while parked on a stereo frame, and that dimensions stay 2W×H through a forced eye stall (flip Vision to Mono mid-play) with the degraded label appearing only after ~4 s. Still open from the issue: a `./scripts/monitor_ndi_logs.sh` capture during a real-playback flap (not parked) to establish which eye starves at what cadence — the "why more frequent in v1.10.0" question stays unanswered until then.
