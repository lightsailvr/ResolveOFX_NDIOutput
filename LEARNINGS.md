...
### 2026-08-30 — vcpkg's dynamic zlib loads fine everywhere except inside Resolve (ticket #21)
**Symptom:** the staged bundle's `NDIOutput.ofx` failed a `LOAD_LIBRARY_SEARCH_SYSTEM32`-only load probe (error 126) — meaning Resolve's Effects Library scan would silently drop the plugin — while every unit test and CI job stayed green.
**Root cause:** vcpkg's default `x64-windows` triplet builds zlib as a DLL (`z.dll`). Windows never searches a DLL's own folder for that DLL's load-time imports, so `z.dll` staged beside the plugin resolves in test runs (the test exe's directory IS searched) but never inside Resolve — the exact loader trap spec decision 12 dodges for the NDI DLL, reintroduced by the build system one dependency over.
**Fix:** pin `VCPKG_TARGET_TRIPLET=x64-windows-static-md` before `project()` (zlib links static; the NDI runtime stays a DLL — its license requires shipping the vendor binary). Guarded forever by `test_plugin_delayload`, which loads the freshly built `.ofx` from a bare temp dir with a System32-only import search and fails on ANY stray load-time import.
**Validated by:** Tier 0 — the probe failed before the triplet change and passes after; full suite green. (Found before the first Tier 1 install, which would otherwise have been a silent no-show.)
**Rule:** an OFX plugin may have NO load-time imports outside System32 — verify with a `LOAD_LIBRARY_SEARCH_SYSTEM32` load of the built binary (that one flag simulates Resolve's scanner), not with tests whose own exe directory hides the problem.

### 2026-08-30 — AddDllDirectory can't rescue a delay-loaded import; preload by full path instead (ticket #21)
**Symptom:** (design, not a runtime failure) spec decision 12 sketched "delay-load + module-relative `AddDllDirectory`" for resolving the bundled NDI DLL. Implementing it showed the sketch doesn't work as written.
**Root cause:** `AddDllDirectory` only affects loads that pass `LOAD_LIBRARY_SEARCH_USER_DIRS`, and the delay-load helper's implicit resolution is a plain by-name `LoadLibrary` with no flags. The only way to make it consult user directories is `SetDefaultDllDirectories` — which rewires DLL resolution for the ENTIRE process, i.e. for every subsequent load Resolve itself performs. A guest must never mutate its host's loader state.
**Fix:** `src/NDIRuntimeLoader.h` instead loads the DLL itself, by full module-relative path (`GetModuleHandleExW(FROM_ADDRESS)` → `GetModuleFileNameW` → sibling path → `LoadLibraryExW`), before the first NDI call: the delay-load helper's later by-name lookup then hits the already-loaded module and never searches disk. System-search fallback keeps machines with a real NDI runtime working; total failure logs both Win32 error codes and leaves the plugin alive as a pass-through (an unresolved delay-load call would instead raise SEH inside Resolve).
...
### 2026-08-31 — Inno's uninstaller re-execs from %TEMP%, so waiting on the process proves nothing (ticket #23)
...
### 2026-08-31 — A CI-built installer carries a plugin that loads but never streams; name it so nobody can ship it (ticket #23)
...
### 2026-09-01 — macOS ships a python3 that isn't one: the CLT shim looped the Timeline (Auto) helper silently (v1.14.1, issue #34)
**Symptom:** field reports against the public installer: Timeline (Auto) never detected the camera clip while Manual Path worked; the log looped `TimelineWatch: helper exited — retrying in 30 s` with no `!`-prefixed helper line before it. The Windows port hit the identical shape the same day (the Store's `python.exe` App Execution Alias stub).
**Root cause:** `/usr/bin/python3` — the watcher's first candidate — is not an interpreter. It links only `libxcselect.dylib`: with the Command Line Tools installed it execs `<developer dir>/usr/bin/python3` (which is why the 2026-08-30 verification on this dev machine passed), and without them it prints `xcode-select: note: No developer tools were found…` to stderr (which the watcher routes to /dev/null), can pop the CLT install dialog, and exits — before the helper's first protocol line, so nothing ever reached the log. A dead child's exit status was never logged either.
**Fix:** ported the Windows approach — the helper is now `src/ndi_timeline_watch.lua` under Resolve's OWN bundled interpreter (`DaVinci Resolve.app/Contents/Libraries/Fusion/fuscript -q -l lua`, located from the host executable via `_NSGetExecutablePath`), so Auto depends on nothing outside Resolve. The Python helper stays as the fuscript-missing fallback, but its candidates (`src/MacTimelineWatch.h`) now resolve the developer-dir python the way the shim would (DEVELOPER_DIR → `/var/db/xcode_select_link` → Xcode.app → CLT) then Homebrew, never spawn `/usr/bin/python3`, and trust a candidate only after it has run `--version` and exited 0 (probe-once, so any future stub at any path is skipped); with nothing usable the health line names the problem once ("no fuscript in the host app bundle and no python3…") instead of looping. The helper-exit log carries `describeWaitStatus()` (code/signal). Review pass also closed two latent lifecycle holes: the pipe ends are now `FD_CLOEXEC` (an unrelated child Resolve forks could otherwise inherit the write end and defeat the EOF wake-up), and `shutdown()` escalates SIGTERM→SIGKILL after 2 s so a helper trapping SIGTERM can't pin plugin unload.
**Validated by:** Tier 0 — `make dev` + `make test` (new `tests/test_mac_timeline_watch.cpp`: fuscript path derivation, candidate order excludes the shim, wait-status wording, /bin/sh spawn/pipe/exit-code round trip incl. close-on-exec, probe-once accept/reject). Standalone harness linking the real watcher: fuscript chain spawns, heartbeats, `helper exited (signal 9) — retrying` after an external SIGKILL, clean shutdown with no orphan. Tier 0.5 — `fuscript -q -l lua src/ndi_timeline_watch.lua` runs standalone on the dev Mac: banner-free, `bmd.scriptapp`/`bmd.wait` present, correct `!` status line while external scripting was off (the Python helper printed the same, so the connection path is shared). Tier 1–2 live verification with external scripting enabled still owed (Matt).
**Rule:** an executable at the canonical interpreter path is not evidence of an interpreter — on macOS check `otool -L` for `libxcselect` — and when Resolve's own `fuscript` can run the helper, prefer it and depend on nothing; always log a dead child's exit status.

### 2026-09-01 — A long-lived port branch forked the version line; retired `windows-port` into `dev` (PR #37)
**Symptom:** the macOS #34 fix and the Windows fuscript work both bumped to 1.14.1 and both added `src/ndi_timeline_watch.lua`, on different branches; every further Mac feature would have re-conflicted the same way, and neither line had CI coverage for the other platform.
**Root cause:** `windows-port` was a long-lived integration branch gated on the Windows release bar (spec decision 2), so `dev` and the port diverged for days while sharing one `VERSION`/`CHANGELOG.md` by design (spec decision 1).
**Fix:** merged `windows-port` → `dev` (a fast-forward: it was a strict superset) before the release bar, then merged `dev` back into the PR head to resolve four text-only conflicts (one unified 1.14.1 CHANGELOG section, both LEARNINGS entries, the watcher header comment, the Lua header comment). The release bar now gates `dev` → `master`; CLAUDE.md, the spec, BUILD.md and the Windows CI triggers (`dev`/`master`/`feature/**`) updated in the follow-up PR.
